class Student:
    """
    Class for students.
    Every student has: a name, list of courses by title, and an average overall grade.
    """
...
def succeeding_students(student_list: list) -> list:
    ...
    """
    return [student for student in student_list if student.average_grade >= 1.0]
...
def sort_by_best_grade(student_list: list) -> list:
    ...
    """
    # Filter out students who are failing (grade < 1.0)
    succeeding = [student for student in student_list if student.average_grade >= 1.0]
    
    return sorted(succeeding, key=lambda student: (-student.average_grade, student.name))
